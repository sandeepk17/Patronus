...
class Java():
    ...
    def check_build(self, repo:str):
        ...
        """
        build = ""
        mvn = Path("%s%s/pom.xml" % (self.config.PATRONUS_DOWNLOAD_LOCATION, repo))
        gradle = Path("%s%s/build.gradle" % (self.config.PATRONUS_DOWNLOAD_LOCATION, repo))
        if mvn.is_file():
            build = "maven"
    ...
    def add_build_tag(self, repo:str):
        try:
            tree = ET.parse("%s%s/pom.xml" % (self.config.PATRONUS_DOWNLOAD_LOCATION, repo))
            root = tree.getroot()
            filename = "%s%s/pom.xml" % (self.config.PATRONUS_DOWNLOAD_LOCATION, repo)
            namespaces = dict(
                [node for _, node in ET.iterparse(filename, events=['start-ns'])])
            for ns in namespaces:
                ET.register_namespace(ns, namespaces[ns])
            root.append(ET.fromstring(self.const.POM_BUILD_TAG))
            tree.write("%s%s/pom.xml" % (self.config.PATRONUS_DOWNLOAD_LOCATION,repo), xml_declaration=True)
        except:
            logging.debug("Failed modified pom.xml for maven project %s" % (repo))
        return
    ...
    def modify_pom_for_findsecbugs(self, repo:str):
        ...
        try:
            ...
            logging.info("successfully modified pom.xml for maven project %s" % (repo))
        except:
            logging.debug("Failed modified pom.xml for maven project %s" % (repo))
        return
    ...
    def modify_gradle_for_findsecbugs(self, repo:str):
        ...
        try:
            ...
            # reading gradle file
            f = open("%s%s/build.gradle" % (self.config.PATRONUS_DOWNLOAD_LOCATION, repo), "r")
            contents = f.readlines()
            f.close()
            
            #apply plugin
            contents.insert(lookup_lineno[0], self.const.FINDSECBUGS_PATTERN_VALUE_1)
            contents.insert(lookup_lineno[0], self.const.DEPENDENCY_CHECK_PATTERN_VALUE_2)

            # writing contents
            f = open("%s%s/build.gradle" % (self.config.PATRONUS_DOWNLOAD_LOCATION, repo), "w")
            ...
            #dependencies {
            contents.insert(lookup_2_lineno[-1] + 2, self.const.FINDSECBUGS_PATTERN_VALUE_2)
            contents.insert(lookup_2_lineno[0] + 1, self.const.DEPENDENCY_CHECK_PATTERN_VALUE_1)
            ...
            f = open("%s%s/build.gradle" % (self.config.PATRONUS_DOWNLOAD_LOCATION, repo), "a")
